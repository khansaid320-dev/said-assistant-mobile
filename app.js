...
async function jinaSearchAnswer(query){
  ...
  try{
    const engines=[
      ['Google','https://www.google.com/search?q='+encodeURIComponent(query)+'&hl=sv'],
      ['Bing','https://www.bing.com/search?q='+encodeURIComponent(query)+'&setlang=sv-se']
    ];
...
function v21EnglishResponse(text){
  ...
  for(const [a,b] of replacements)x=x.split(a).join(b);
  return x;
}
...
async function send(){
  ...
  const pending=document.createElement('div');pending.id=pendingId;pending.className='bubble assistant pending-answer';pending.textContent='⏳ Jag förstår frågan och letar efter ett relevant svar…';
  $('#conversation').appendChild(pending);$('#conversation').scrollTop=$('#conversation').scrollHeight;setAnswerStatus('🔎 Söker och analyserar…');
  try{
    ...
    else add('Jag kunde inte slutföra svaret just nu. Försök igen så fortsätter jag i samma chatt.','assistant');
  }catch(e){
    pending.remove();
    add('Jag fick ett tekniskt fel när jag försökte hitta svaret. Jag har inte raderat din fråga eller chatthistorik. Försök igen så fortsätter vi härifrån.','assistant');
...
$('#profileName').value=S.get('profileName','Saidkhan'); $('#profileSurname').value=S.get('profileSurname','Safizadah'); $('#profilePersonalNumber').value=S.get('profilePersonalNumber','20031114-2434'); $('#birthDate').value=S.get('birthDate','2003-11-14'); $('#profileLocation').value=S.get('profileLocation',''); $('#profileAddress').value=S.get('profileAddress',''); $('#profileJob').value=S.get('profileJob','');
...
$('#cvFile').onchange=()=>{const f=$('#cvFile').files[0];if(!f)return;const r=new FileReader();r.onload=()=>{$('#cvText').value=String(r.result||'');S.set('cvText',$('#cvText').value);$('#cvAnalysis').textContent='CV importerat. Tryck ”Spara CV” när du är nöjd.'};r.readAsText(f)};
$('#saveCv').onclick=()=>{S.set('cvText',$('#cvText').value);add('CV sparat lokalt. Nästa gång jag söker jobb använder jag det automatiskt.');};
$('#analyzeCv').onclick=()=>$('#cvAnalysis').textContent=analyzeCV($('#cvText').value);
$('#cvPdf').onclick=()=>download('CV.pdf',pdf($('#cvText').value));
...
async function handleChatAttachment(){
  ...
  try{
    if(f.type.startsWith('image/')){
      const text=await window.SaidImageTools.read(f,m=>{if(status&&m?.progress)status.textContent=`⏳ Läser bild… ${Math.round(m.progress*100)}%`;});
...
async function analyzeSelectedImage(){
  ...
  try{
    const text=await window.SaidImageTools.read(f,m=>{if(m?.progress)$('#imageStatus').textContent=`⏳ Läser bilden… ${Math.round(m.progress*100)}%`;});
...
$('#analyzeImage')?.addEventListener('click',analyzeSelectedImage);
$('#sendImageText')?.addEventListener('click',()=>{const text=$('#imageText').value.trim();if(!text){$('#imageStatus').textContent='Det finns ingen läst text att lägga i chatten.';return}$('#command').value=`Jag har skickat en bild. Här är texten som lästes ut:\n\n${text}\n\nKan du förklara och svara på frågorna i texten tydligt, en i taget?`;send();});

$('#documentFile').onchange=()=>{};
$('#openFile').onclick=()=>{const f=$('#documentFile').files[0];if(!f){$('#docStatus').textContent='Välj en fil först.';return}const ext=f.name.toLowerCase().split('.').pop();if(ext==='pdf'){window.open(URL.createObjectURL(f),'_blank','noopener');$('#docStatus').textContent='PDF öppnad. Den fristående mobilversionen visar PDF-filen men låtsas inte att den har läst texten.';return}if(['png','jpg','jpeg','webp'].includes(ext)){$('#documentText').value='Bild: '+f.name+'\nStorlek: '+Math.round(f.size/1024)+' KB';$('#docStatus').textContent='Bild vald. Bildförståelse kräver en bildmodell.';return}const r=new FileReader();r.onload=()=>{$('#documentText').value=String(r.result||'');$('#docStatus').textContent='Öppnad: '+f.name};r.readAsText(f)};
$('#saveText').onclick=()=>download('dokument_redigerat.txt',new Blob([$('#documentText').value],{type:'text/plain;charset=utf-8'}));$('#makePdf').onclick=()=>download('dokument.pdf',pdf($('#documentText').value));
...
if(navigator.storage?.persist)navigator.storage.persist().catch(()=>{});if('serviceWorker'in navigator)navigator.serviceWorker.register('./service-worker.js').then(r=>r.update()).catch(()=>{});
