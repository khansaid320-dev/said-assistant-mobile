...
async function answer(t){
  ...
  if(/hur gammal är jag|hur gammal e jag/.test(s)){const a=age(S.get('birthDate',''));return a==null?'Spara ett giltigt födelsedatum under Min profil.':`Du är ${a} år gammal. Du är född ${S.get('birthDate','')}.`}
...
/* ========================================================================
   FINAL V27.1 — COMPLETE CV + DOCUMENT WORKSPACE
   Keeps the existing Said Assistant features, but makes CV/document handling
   a real mobile workspace: import, preview, extraction, editing, autosave,
   undo, classification and new-PDF export. Everything remains local.
   ======================================================================== */
(function(){
  'use strict';
  const $v27=s=>document.querySelector(s);
  const V='saidAssistantWorkspaceV27';
  let cvOriginal='', docOriginal='', currentDocName='dokument', currentDocFile=null;
  let cvTimer=null, docTimer=null;

  function sayV(sv,en){return window.SaidLanguage?.get?.()==='en'?en:sv}
  ...
  function extOf(f){return String(f?.name||'').toLowerCase().split('.').pop()||''}
  function typeLabel(f,forced){
    if(forced&&forced!=='auto') return ({cv:'CV',letter:'Personligt brev',application:'Ansökan',job:'Jobbannons',other:'Dokument'}[forced]||'Dokument');
    const e=extOf(f);
    if(e==='pdf')return 'PDF';
    if(e==='doc'||e==='docx')return 'Word';
    if(/^png|jpe?g|webp|gif|bmp|heic$/.test(e))return 'Bild';
    if(e==='txt'||e==='md')return 'Text';
    if(e==='csv')return 'CSV';
    if(e==='json')return 'JSON';
    if(e==='html'||e==='htm')return 'HTML';
    return e?e.toUpperCase():'Dokument';
  }
  function setInfo(id,text){const e=$v27('#'+id);if(e)e.textContent=text}
  function setState(id,text){const e=$v27('#'+id);if(e)e.textContent=text}
  function clearPreview(id){const e=$v27('#'+id);if(e){e.innerHTML='';e.hidden=true}}
  function revokePreview(e){const old=e?.dataset?.objectUrl;if(old)try{URL.revokeObjectURL(old)}catch{}}
  function showPreview(containerId,file){
    const box=$v27('#'+containerId);if(!box)return;
    revokePreview(box);box.innerHTML='';box.hidden=false;
    const ext=extOf(file), url=URL.createObjectURL(file);box.dataset.objectUrl=url;
    if(ext==='pdf'){
      const frame=document.createElement('iframe');frame.src=url+'#view=FitH';frame.title='PDF-förhandsvisning';box.appendChild(frame);
      const n=document.createElement('div');n.className='preview-note';n.textContent=sayV('Original-PDF visas här. Den redigerbara texten finns under förhandsvisningen.','The original PDF is shown here. Editable text is below the preview.');box.appendChild(n);
    }else if(file.type?.startsWith('image/')||/^(png|jpe?g|webp|gif|bmp|heic)$/.test(ext)){
      const img=document.createElement('img');img.src=url;img.alt=file.name;box.appendChild(img);
      const n=document.createElement('div');n.className='preview-note';n.textContent=sayV('Originalbilden visas. OCR kan göra texten redigerbar.','The original image is shown. OCR can make its text editable.');box.appendChild(n);
    }else if(ext==='docx'||ext==='doc'){
      const n=document.createElement('div');n.className='preview-note';n.innerHTML='<strong>📄 '+String(file.name).replace(/[<>&"]/g,'')+'</strong><br>'+sayV('Word-filen kan läsas till redigerbar text. Originalfilen ändras aldrig.','The Word file can be read into editable text. The original file is never changed.');
      const a=document.createElement('a');a.href=url;a.target='_blank';a.rel='noopener';a.textContent=sayV('Öppna originalfil','Open original file');n.appendChild(document.createElement('br'));n.appendChild(a);box.appendChild(n);
    }else{
      const n=document.createElement('div');n.className='preview-note';n.textContent=sayV('Filen är öppnad som ett redigerbart dokument.','The file is opened as an editable document.');box.appendChild(n);
    }
  }
  async function readAnyV27(file,progress){
    const e=extOf(file);
    if(e==='pdf' && window.SaidDocumentEditorV26?.readPdfText){
      let t=await window.SaidDocumentEditorV26.readPdfText(file,progress);
      if(!t && window.SaidDocumentEditorV26?.read)t=await window.SaidDocumentEditorV26.read(file,progress);
      return t||'';
    }
    if((file.type||'').startsWith('image/') && window.SaidImageToolsV26?.read){
      return await window.SaidImageToolsV26.read(file,progress)||'';
    }
    if(window.SaidDocumentEditorV26?.read){
      const t=await window.SaidDocumentEditorV26.read(file,progress);if(t)return t;
    }
    if(['txt','md','csv','json','html','htm','xml','rtf'].includes(e))return await file.text();
    return '';
  }
  function storeLocal(key,text,meta){
    try{localStorage.setItem(V+':'+key,JSON.stringify({text:String(text||''),meta:meta||{},savedAt:new Date().toISOString()}))}catch{}
  }
  function loadLocal(key){
    try{const x=JSON.parse(localStorage.getItem(V+':'+key)||'null');return x||null}catch{return null}
  }
  function downloadV27(name,blob){
    if(!blob)return;
  ...
  function pdfV27(text,title){
    if(window.SaidDocumentEditorV26?.makePdf)return window.SaidDocumentEditorV26.makePdf(text,title);
    if(window.SaidMakePdf)return window.SaidMakePdf(text);
    return null;
  }

  async function importCvV27(){
    const f=$v27('#cvFile')?.files?.[0];if(!f){setState('cvEditState',sayV('Välj ett CV först.','Choose a CV first.'));return}
    showPreview('cvPreview',f);setInfo('cvFileInfo',`${f.name} · ${typeLabel(f)} · ${fmtBytes(f.size)}`);
    setState('cvEditState',sayV('⏳ Läser CV…','⏳ Reading CV…'));
    try{
      const t=await readAnyV27(f,(p,total)=>setState('cvEditState',sayV(`⏳ Läser sida ${p} av ${total}…`,`⏳ Reading page ${p} of ${total}…`)));
      if(t){
        cvOriginal=t;$v27('#cvText').value=t;storeLocal('cv',t,{name:f.name,type:typeLabel(f)});
        setState('cvEditState',sayV('✅ CV öppnat. Du kan redigera allt i rutan.','✅ CV opened. You can edit everything in the editor.'));
        if($v27('#cvAnalysis'))$v27('#cvAnalysis').textContent=sayV('CV:t är importerat och redigerbart. Ändra texten och tryck Spara CV.','The CV is imported and editable. Change the text and press Save CV.');
      }else{
        setState('cvEditState',sayV('⚠️ Filen kunde inte omvandlas till redigerbar text. Originalet kan fortfarande visas.','⚠️ The file could not be converted to editable text. The original can still be previewed.'));
      }
    }catch(err){console.error(err);setState('cvEditState',sayV('❌ Kunde inte läsa CV-filen.','❌ Could not read the CV file.'))}
  }
  function saveCvV27(){
    const t=$v27('#cvText')?.value||'';storeLocal('cv',t,{name:$v27('#cvFile')?.files?.[0]?.name||'CV'});
    cvOriginal=t;setState('cvEditState',sayV('✅ CV-ändringarna är sparade lokalt automatiskt.','✅ CV changes are saved locally automatically.'));
    if(typeof add==='function')add(sayV('CV sparat lokalt.','CV saved locally.'));
  }
  function undoCvV27(){
    $v27('#cvText').value=cvOriginal||'';storeLocal('cv',cvOriginal,{name:'CV'});
    setState('cvEditState',sayV('↩️ CV:t är tillbaka till senaste sparade version.','↩️ CV restored to the latest saved version.'));
  }
  function createCvPdfV27(){
    const t=$v27('#cvText')?.value||'';if(!t.trim()){setState('cvEditState',sayV('CV:t är tomt.','The CV is empty.'));return}
    storeLocal('cv',t,{name:'CV'});const b=pdfV27(t,'CV');if(b)downloadV27('CV_uppdaterat.pdf',b);
    setState('cvEditState',sayV('📄 Ny PDF skapad från den redigerade CV-versionen.','📄 New PDF created from the edited CV.'));
  }
  function downloadCvTextV27(){
    const t=$v27('#cvText')?.value||'';if(t)downloadV27('CV_uppdaterat.txt',new Blob([t],{type:'text/plain;charset=utf-8'}));
  }
  async function openDocV27(){
    const f=$v27('#documentFile')?.files?.[0];if(!f){setState('docEditState',sayV('Välj ett dokument först.','Choose a document first.'));return}
    currentDocFile=f;currentDocName=(f.name||'dokument').replace(/\.[^.]+$/,'');
    showPreview('documentPreview',f);setInfo('docFileInfo',`${f.name} · ${typeLabel(f,$v27('#documentType')?.value)} · ${fmtBytes(f.size)}`);
    setState('docEditState',sayV('⏳ Läser dokument…','⏳ Reading document…'));
    try{
      const t=await readAnyV27(f,(p,total)=>setState('docEditState',sayV(`⏳ Läser sida ${p} av ${total}…`,`⏳ Reading page ${p} of ${total}…`)));
      if(t){
        docOriginal=t;$v27('#documentText').value=t;storeLocal('doc',t,{name:f.name,type:typeLabel(f,$v27('#documentType')?.value)});
        setState('docEditState',sayV('✅ Dokument öppnat. Redigera texten och spara.','✅ Document opened. Edit the text and save.'));
        statusV27();
      }else{
        setState('docEditState',sayV('⚠️ Ingen redigerbar text hittades. För skannade bilder kan OCR behöva köras separat.','⚠️ No editable text found. Scanned images may need OCR separately.'));
      }
    }catch(err){console.error(err);setState('docEditState',sayV('❌ Kunde inte läsa dokumentet.','❌ Could not read the document.'))}
  }
  function statusV27(){
    const t=$v27('#documentText')?.value||'';
    setState('docEditState',t.trim()?sayV('✏️ Redigerbar text klar. Ändringar sparas lokalt.','✏️ Editable text ready. Changes are saved locally.') : sayV('Välj ett dokument.','Choose a document.'));
  }
  function saveDocV27(){
    const t=$v27('#documentText')?.value||'';storeLocal('doc',t,{name:currentDocName,type:$v27('#documentType')?.value||'auto'});
    statusV27();
  }
  function createDocPdfV27(){
    const t=$v27('#documentText')?.value||'';if(!t.trim()){setState('docEditState',sayV('Det finns ingen text att skapa PDF av.','There is no text to create a PDF from.'));return}
    saveDocV27();const b=pdfV27(t,currentDocName||'dokument');if(b)downloadV27((currentDocName||'dokument')+'_redigerat.pdf',b);
    setState('docEditState',sayV('📄 Ny PDF skapad från dina ändringar. Originalfilen är oförändrad.','📄 New PDF created from your changes. The original file is unchanged.'));
  }
  async function makeImagePdfV27(){
    const f=currentDocFile;if(!f||!(f.type||'').startsWith('image/'))return false;
    const img=await new Promise((res,rej)=>{const i=new Image();i.onload=()=>res(i);i.onerror=rej;i.src=URL.createObjectURL(f)});
    const c=document.createElement('canvas'),max=1500,scale=Math.min(1,max/Math.max(img.width,img.height));c.width=Math.max(1,Math.round(img.width*scale));c.height=Math.max(1,Math.round(img.height*scale));
    c.getContext('2d').drawImage(img,0,0,c.width,c.height);const data=c.toDataURL('image/jpeg',.92);
    // A simple PDF wrapper using the existing text PDF is intentionally not used
    // here because it would destroy the image. Offer the original image instead
    // when browser PDF embedding is unavailable.
    downloadV27((currentDocName||'dokument')+'.jpg',await (await fetch(data)).blob());URL.revokeObjectURL(img.src);return true;
  }
  function newDocV27(){
    docOriginal='';currentDocName='dokument';currentDocFile=null;
    if($v27('#documentText'))$v27('#documentText').value='';
    if($v27('#documentFile'))$v27('#documentFile').value='';
    clearPreview('documentPreview');setInfo('docFileInfo','Nytt dokument.');statusV27();
  }
  function autoSaveCv(){clearTimeout(cvTimer);cvTimer=setTimeout(()=>{const t=$v27('#cvText')?.value||'';storeLocal('cv',t,{name:'CV',auto:true})},500)}
  function autoSaveDoc(){clearTimeout(docTimer);docTimer=setTimeout(()=>{const t=$v27('#documentText')?.value||'';storeLocal('doc',t,{name:currentDocName,auto:true})},500)}
  function restoreV27(){
    const cv=loadLocal('cv');if(cv?.text&&$v27('#cvText')){$v27('#cvText').value=cv.text;cvOriginal=cv.text;setState('cvEditState',sayV('💾 Senast sparade CV-text återställd.','💾 Last saved CV text restored.'))}
    const doc=loadLocal('doc');if(doc?.text&&$v27('#documentText')){$v27('#documentText').value=doc.text;docOriginal=doc.text;currentDocName=doc.meta?.name||'dokument';statusV27()}
  }
  function wireV27(){
    // Override older handlers so the final workspace is authoritative.
    const cf=$v27('#cvFile');if(cf)cf.onchange=importCvV27;
    const sc=$v27('#saveCv');if(sc)sc.onclick=saveCvV27;
    const ae=$v27('#analyzeCv');if(ae)ae.onclick=()=>{if(typeof analyzeCV==='function')$v27('#cvAnalysis').textContent=analyzeCV($v27('#cvText').value)};
    const cp=$v27('#cvPdf');if(cp)cp.onclick=createCvPdfV27;
    const dc=$v27('#downloadCv');if(dc)dc.onclick=downloadCvTextV27;
    const oe=$v27('#openCvEditor');if(oe)oe.onclick=()=>{$v27('#cvText')?.focus();$v27('#cvText')?.scrollIntoView({behavior:'smooth',block:'center'});setState('cvEditState',sayV('✏️ Redigeringsläge aktivt.','✏️ Editing mode active.'))};
    const uc=$v27('#undoCv');if(uc)uc.onclick=undoCvV27;
    $v27('#cvText')?.addEventListener('input',autoSaveCv);

    const of=$v27('#openFile');if(of)of.onclick=openDocV27;
    const ed=$v27('#editDocument');if(ed)ed.onclick=()=>{$v27('#documentText')?.focus();$v27('#documentText')?.scrollIntoView({behavior:'smooth',block:'center'});statusV27()};
    const st=$v27('#saveText');if(st)st.onclick=saveDocV27;
    const mp=$v27('#makePdf');if(mp)mp.onclick=createDocPdfV27;
    const sp=$v27('#saveEditedPdf');if(sp)sp.onclick=createDocPdfV27;
    const nd=$v27('#newDocument');if(nd)nd.onclick=newDocV27;
    const ud=$v27('#undoDocument');if(ud)ud.onclick=()=>{$v27('#documentText').value=docOriginal||'';saveDocV27();setState('docEditState',sayV('↩️ Ändringarna har ångrats.','↩️ Changes undone.'))};
    $v27('#documentText')?.addEventListener('input',autoSaveDoc);
    $v27('#documentType')?.addEventListener('change',()=>{if(currentDocFile)setInfo('docFileInfo',`${currentDocFile.name} · ${typeLabel(currentDocFile,$v27('#documentType').value)} · ${fmtBytes(currentDocFile.size)}`)});
    restoreV27();
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',wireV27);else wireV27();
})();
